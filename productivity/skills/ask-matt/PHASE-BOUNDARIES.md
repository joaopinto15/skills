...
- swapping to a **different tool** (Claude → Codex),
...
**5. Otherwise, `/compact`.** Relevant context, same tool, same directory, and you need to stay in the loop: this is where the tree lands, and it lands here often. Pass it an instruction (`/compact we're going to QA this area`) so the summary keeps what the next phase needs.
